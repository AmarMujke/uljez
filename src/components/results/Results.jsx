...
export default function Results({ players, votes, leaderboard = {}, updateLeaderboard, resetGame, nextRound }) {
  const { t } = useLanguage();
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-purple-900 to-purple-600 p-6">
      <div className="w-full max-w-2xl grid grid-cols-1 gap-6">
        <div className="bg-gradient-to-br from-purple-700 to-purple-500 rounded-3xl shadow-2xl p-6 border-4 border-yellow-400">
          <h2 className="text-4xl font-extrabold text-yellow-400 mb-4">{t.gameOver}</h2>

          <p className={`text-2xl font-bold mb-4 ${impostorCaught ? "text-green-300" : "text-red-400"}`}>
            {impostorCaught ? t.impostorCaught : t.impostorWins}
          </p>

          <div className="bg-purple-800 text-yellow-300 rounded-xl shadow-lg p-4 mb-4">
            <h3 className="text-lg font-semibold mb-2">{t.impostorWas}</h3>
          ...
          <div className="flex gap-3 mt-4">
            <button
              onClick={nextRound}
              className="flex-1 bg-purple-800 text-yellow-300 px-4 py-2 rounded-xl font-semibold shadow hover:scale-105 transition transform"
            >
            ...
            <button
              ...
              className="flex-1 bg-red-600 text-white px-4 py-2 rounded-xl font-semibold shadow hover:scale-105 transition transform"
            >
              {t.exitGame || "Exit"}
            </button>
          </div>
