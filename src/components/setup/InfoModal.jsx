  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-gradient-to-br from-yellow-300/40 to-yellow-200/30 backdrop-blur-lg p-6 rounded-xl shadow-2xl w-96 max-w-[90%] text-center border border-purple-600/50">
        <h2 className="text-2xl font-bold mb-4 text-purple-800 uppercase">
          {title}
        </h2>

        <div className="bg-black/20 rounded-lg p-3">
          <p className="text-yellow-300 font-bold mb-6 text-left">{description}</p>
        </div>
        ...
        <div className="flex justify-between items-center mt-6">
          ...
          {stepIndex === steps.length - 1 ? (
            <Button
              onClick={onClose}
              className="px-4 py-2 bg-purple-800/70 text-yellow-300 rounded-lg hover:scale-105 transition"
            >
